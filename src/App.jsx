...
function App() {
  ...
  const handleSubmitRole = async () => {
    ...
    try {
      ...
      let edit = `${baseUrl}${endpointPut}`

      const response = await fetch(selectedModules?.name || selectedModules?.roleID ? edit : post, {
        method: selectedModules?.name || selectedModules?.roleID ? 'PUT' : 'POST',
        headers: {
      ...
      data && alert("Role created successfully");
  ...
  return (
    <main className="container hero">
      {/* button */}
      <div className='margin-bottom' style={{ marginBottom: 30, display: 'flex', justifyContent: 'flex-end' }}>
        <button className="btn" onClick={handleSubmitRole}>{loading ? 'loading...' : (selectedModules?.name || selectedModules?.roleID) ? 'Update Role' : 'Create Role'}</button>
      </div>
      ...
      {/* TABLE */}
      <CoreTable columns={columns} contentLength={shownModules?.length} isLoading={loading}>
            ...
            <td style={{ padding: '4px 10px', textAlign: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'stretch' }}>
                {ele.subModules?.map((sub, idx) => (
                  <div key={idx} style={{ padding: '10px', border: "1px solid rgba(255, 255, 255, 0.1)", borderRadius: 8, whiteSpace: 'nowrap' }}>{sub?.moduleName}</div>
                ))}
              </div>
            </td>
            {/* Permissions */}
            <td>
              <div className="perm-list" style={{ marginTop: 4 }}>
                {ele?.subModules?.map((sub) => (
                  <div key={sub.moduleID} style={{ display: 'flex', flexDirection: 'row', gap: 10, fontSize: 12, flexWrap: 'wrap' }}>
                    {sub?.permissions?.map((permission, idx) => (
                      <label key={idx} className={`perm-pill ${permission.isAllowed ? 'active' : ''}`}>
                        <input
                          type="checkbox"
                          defaultChecked={permission.isAllowed}
                          onChange={(e) => togglePermission(permission?.permissionId, e.target.checked)}
                        />
